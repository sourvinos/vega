...
export class VehicleFormComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        return forkJoin(sources).subscribe(result => {
            ...
            if (this.vehicle.id) {
                this.setVehicle(result[2] as { id: number, make: KeyValuePair, model: KeyValuePair, isRegistered: boolean, features: [], contact: { name: string, email: string, phone: string }, lastUpdate: string });
            }
        }, error => {
            if (error.status == 404) {
